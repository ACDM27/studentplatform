...
export const useCourseService = () => {
  ...
  const loadCourses = async (forceRefresh = false) => {
    ...
    try {
      ...
      // 转换数据格式
      courses.value = courseData.map((item: ICourseItem) => {
        const { type, typeText } = parseCourseType(item.type || '')
        const day = parseTimeToDay(item.classTime || '')
        ...
        return {
          ...
          time: item.classTime || '时间待定',
          location: item.classroom || '地点待定',
          studentsCount: item.students || 0,
          description: item.coursecontent || '暂无课程描述',
          day,
          credit: item.credit,
