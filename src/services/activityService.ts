...
import { getActivities, getActivityById, postActivity, putActivity, deleteActivity } from '../apis/index'
...
export const useActivityService = () => {
  ...
  const loadActivities = async (forceRefresh = false) => {
    ...
    try {
      ...
      let activityData: IActivityItem[] = []
      
      // 根据 IGetActivitiesResp 接口定义，response 的结构是 { data: Array<IActivityItem>, meta: {...} }
  ...
  const getActivity = async (id: string | number) => {
    try {
      const params: IGetActivityByIdParams = { id }
      const response = await getActivityById(params)
      
      // 根据 IGetActivityByIdResp 接口定义，response 的结构是 { data: IActivityItem }
