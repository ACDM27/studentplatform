...
export const exampleChatWithStudentData = async () => {
  try {
    const response = await chatWithAI({
      question: '请分析我的学习成果和表现',
      // student_id: 不提供时会自动获取当前学生
      context: '学习成果分析' // 上下文信息
    })
    
    console.log('AI分析结果:', response.data.response)
    return response
  } catch (error) {
...
export const exampleChatWithoutStudentData = async () => {
  try {
    const response = await chatWithAI({
      question: '什么是机器学习？',
      // student_id: 可选，不提供时会自动获取当前学生
      context: '通用知识问答' // 上下文信息
    })
    
    console.log('AI回答:', response.data.response)
    return response
  } catch (error) {
    console.error('AI聊天失败:', error)
    throw error
