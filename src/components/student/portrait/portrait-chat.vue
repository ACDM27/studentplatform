  ...
  try {
    ...
    if (messageIndex !== -1) {
      chat_messages.value[messageIndex] = {
        ...aiMessage,
        content: resp?.data?.response || resp?.data?.message || '抱歉，暂时无法获取回复。',
        loading: false,
        data: resp?.data?.structured_data || null
      }
    }
  } catch (error) {
...
const regenerateMessage = async (messageId: string) => {
  ...
  try {
    ...
    chat_messages.value[messageIndex] = {
      ...chat_messages.value[messageIndex],
      content: response.data.response || '抱歉，我暂时无法回答这个问题。',
      loading: false,
      data: response.data.structured_data || null
    }
