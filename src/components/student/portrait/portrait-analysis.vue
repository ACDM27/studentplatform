...
const fetchAIAnalysis = async (): Promise<void> => {
  try {
    ...
    const resp = await chatWithAI({
      question: '请基于我的画像数据返回JSON，包含comprehensiveScore, ranking, strengthArea',
      student_id: student_id.value,
      context: '画像分析请求'
    })
    const text = resp?.data?.response || resp?.data?.message || ''
    let parsed: any = null
    try {
...
const fetchAchievementCounts = async (): Promise<void> => {
  try {
    ...
    for (const item of list) {
      const tRaw = item?.type || item?.category || item?.attributes?.type || ''
